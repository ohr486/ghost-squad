...
export default function Home() {
  ...
  const handleDelete = async (e: React.MouseEvent, idToDelete: string) => {
    // 重要: 親のボタン(選択アクション)が反応しないようにイベントを止める
    e.stopPropagation();
    ...
    try {
      ...
      if (res.ok || res.status === 204) {
        // 1. UIの履歴リストから即座に消す
        const newHistory = history.filter((m) => m.id !== idToDelete);
        setHistory(newHistory);

        // 2. もし「今見ているミッション」を消した場合は、画面をリセットするか最新に切り替える
        const currentId = missionData.id || missionData.mission_id;
        if (currentId === idToDelete) {
          if (newHistory.length > 0) {
            setMissionData(newHistory[0]); // 残っている最新を表示
          } else {
            // 全消ししてしまった場合
            setMissionData({
               mission_id: null,
               logs: ["WAITING FOR NEW ORDER..."],
               tasks: []
            });
          }
        }
                    ...
                    {history.map((m: any) => {
                        ...
                        return (
                            <button
                                ...
                                className={`w-full text-left p-3 rounded-lg text-xs transition-all border group relative ${
                                    isSelected 
                                    ? "bg-blue-50 border-blue-200 ring-1 ring-blue-300" 
                                    : "bg-white border-slate-100 hover:bg-slate-50 hover:border-slate-300"
                                }`}
                            >
                                <div className="font-bold truncate mb-1 text-slate-700 pr-6">
