...
from ..schemas import Inquiry, Story, InquiryResult
from ..api import StoryUpdateRequest
from ..enums import StoryCategory, StoryPattern
from ..export import KanbanSystem, ExportResult, SyncStatus, NotificationSettings
...
class InquiryServiceProtocol(Protocol):
    ...
    async def request_clarification(self, inquiry_id: str, questions: List[str]) -> None:
        ...
...
class StoryServiceProtocol(Protocol):
    ...
    async def get_story_history(self, story_id: str) -> List[Dict[str, Any]]:  # StoryVersion
        ...


class ExportServiceProtocol(Protocol):
    async def export_to_kanban(self, stories: List[Story], target_system: KanbanSystem) -> ExportResult:
        ...
...
class NotificationServiceProtocol(Protocol):
    async def send_story_completion_notification(self, user_id: str, stories: List[Story]) -> None:
        ...
    ...
    async def configure_notification_settings(self, user_id: str, settings: NotificationSettings) -> None:
        ...