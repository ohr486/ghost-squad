...
async def create_inquiry(
    inquiry_request: InquiryCreateRequest, db: Session = Depends(get_db)
) -> InquiryResponse:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="データベースエラーが発生しました"
        )
    except ValidationError as e:
        db.rollback()
        logger.error(
            f"Validation error while creating inquiry: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="データの検証に失敗しました"
        )
    except Exception as e:
        db.rollback()
        logger.error(
            f"Unexpected error while creating inquiry: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="予期しないエラーが発生しました"
        )
...
async def get_inquiries(
    ...
    limit: int = Query(
        ...
        ge=1,
        le=1000,
        description="Maximum number of inquiries to return (1-1000)",
    ),
    offset: int = Query(
        default=0, ge=0, description="Number of inquiries to skip (non-negative)"
    ),
    ...
    try:
        ...
        # Convert to response models using list comprehension
        # ORM objects can be passed directly
        return [InquiryResponse.model_validate(inquiry) for inquiry in inquiries]

    except SQLAlchemyError as e:
        logger.error(
            f"Database error while retrieving inquiries: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="データベースエラーが発生しました"
        )
        ...
        logger.error(
            f"Validation error while processing inquiries: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="データの検証に失敗しました"
        )
        ...
        logger.error(
            f"Unexpected error while retrieving inquiries: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="予期しないエラーが発生しました"
        )
...
async def get_inquiry(
    ...
    try:
        ...
        logger.error(
            f"Database error while retrieving inquiry {inquiry_id}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="データベースエラーが発生しました"
        ...
        logger.error(
            f"Validation error while processing inquiry {inquiry_id}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="データの検証に失敗しました"
        ...
        logger.error(
            f"Unexpected error while retrieving inquiry {inquiry_id}: {str(e)}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="予期しないエラーが発生しました"
